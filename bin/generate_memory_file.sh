...
function ecompile {
    ...
    rm -f $ELF_FILE
    "$ASSEMBLER" -mcmodel=medany -static -fno-common -ffreestanding -nostartfiles -nostdlib -march=$ARCH -mabi=ilp32 -Ofast -flto -Wall -Wextra -Wno-unused -T$LINK_FILE $START_FILE "${WORK_DIR}/$(basename $IN_FILE)" -o "$ELF_FILE" -static-libgcc -lgcc -Wl,--no-relax
